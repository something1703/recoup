...
// Staggers the hero's children one after another — the case being laid out
// on the table piece by piece, not everything landing at once.
const heroStagger = {
  hidden: {},
...
export default function Home() {
  return (
    <div>
      {/* Hero — evidence room. Headline + CTA guaranteed above the fold: the
          docket bar and nav are fixed-height, the rest of this flex column
          fills and centers whatever height remains (one real viewport). */}
      <header className="bg-evidence text-paper flex flex-col min-h-screen">
        <div className="shrink-0">
          <DocketBar dark />
          <Nav dark />
        </div>
        <motion.div
          className="case-file flex-1 flex flex-col justify-center py-10"
        ...
        >
          ...
          <motion.p variants={heroItem} className="mt-4 md:mt-5 max-w-2xl text-base md:text-xl font-bold text-paper">
          ...
          <motion.div variants={heroItem} className="mt-6 md:mt-8 flex flex-wrap items-center gap-3 md:gap-4">
            <StampButton href={COCKPIT_URL} variant="primary">
              OPEN THE LIVE COCKPIT →
            </StampButton>
            <StampButton href="/how-it-works" variant="secondary" external={false}>
              HOW THE INVESTIGATION WORKS
            </StampButton>
          </motion.div>
          <motion.div variants={heroItem} className="mt-4 md:mt-6 text-xs md:text-sm text-paper/60 hidden sm:block">
            ↓ four sources, computed evidence, a human sign-off — the full case file below
          </motion.div>
        </motion.div>
      </header>

      {/* Explanatory follow-through + live number, still in the evidence room but no longer gating the fold */}
      <section className="bg-evidence text-paper border-t border-paper/10">
        <div className="case-file py-10 md:py-14 flex flex-col md:flex-row md:items-center gap-8">
          <p className="max-w-2xl text-base md:text-lg text-paper/85 leading-relaxed">
      ...
      <section className="case-file py-16 md:py-24 grid md:grid-cols-[1fr_260px] gap-10 items-start">
        ...
        <Reveal delay={0.15} y={0} className="border-2 border-ink bg-paper p-5 font-type -rotate-1">
          <div className="text-[10px] tracking-[0.2em] text-ink/60 mb-2">EXHIBIT — SOURCE DATA</div>
          <motion.div
            className="text-4xl font-bold tabular-nums text-stamp"
      ...
      <section className="case-file py-16 md:py-24">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { tag: "TENANT 01 · B2B SAAS", name: "Arcline Software", desc: "Failed-payment desk — card-level vs. platform-level, safe retry vs. human escalation." },
            { tag: "TENANT 02 · SUBSCRIPTION RETAIL", name: "Ferro Commerce", desc: "Refund-abuse desk — serial abuse vs. a product-failure wave, quantified before any ticket." },
            { tag: "TENANT 03 · CONSUMER TELECOM", name: "Meridian Telecom", desc: "Account-health desk — 7,043 real subscribers, scored against a real held-out outcome." },
          ].map((t, i) => (
            <Reveal key={t.name} delay={i * 0.12}>
              <motion.div whileHover={{ y: -4 }} className="border border-ink/25 p-5 bg-paper-dim h-full">
                <div className="text-[10px] tracking-[0.2em] text-notary mb-2">{t.tag}</div>
                <div className="font-stamp text-lg mb-2">{t.name}</div>
                <div className="text-sm text-ink/80">{t.desc}</div>
              </motion.div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="bg-evidence text-paper py-16 md:py-24">
        <div className="case-file">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-sm text-paper/85">
            {[
              { title: "Qodo-reviewed", body: "Every pull request is reviewed before it merges. A human — never the agent — performs the merge." },
              { title: "DRY_RUN by default", body: "Rehearsal runs never inflate a number a stakeholder reads as real recovered revenue." },
              { title: "Least-privilege, always", body: "The agent's database role, the tool server's own role, and the evaluation role never overlap." },
            ].map((c, i) => (
              <Reveal key={c.title} delay={i * 0.12}>
                <div className="font-stamp text-lg mb-2 text-paper">{c.title}</div>
                {c.body}
              </Reveal>
            ))}
          </div>
      ...
      <section className="case-file py-20 md:py-28 text-center">
        <Reveal>
          ...
          <StampButton href="https://github.com/something1703/recoup" variant="primary" className="!px-8 !py-4">
            OPEN THE REPOSITORY →
          </StampButton>
        </Reveal>
      </section>
